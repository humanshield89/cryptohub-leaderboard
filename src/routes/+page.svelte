<script lang="ts">
	...
	import { getHubDropPrize } from '../utils/utils';
	import StatCard from '../component/stat-card.svelte';
	...
	$: totalHUBWon = sortedStats?.reduce((acc, curr) => {
		return acc + getHubDropPrize(curr.rarities);
	}, 0);
	const totalUHBAvailable = 519800;
...
<div class="flex flex-col p-0 items-center max-w-7xl gap-4 justify-center w-full m-auto mb-12">
	...
	<div class="grid grid-cols-2 md:grid-cols-4 w-full gap-2 md:gap-4 px-2 my-4">
		<StatCard
			label={'$HUB Prizes Left (' +
				((100 * (totalUHBAvailable - totalHUBWon)) / totalUHBAvailable).toFixed(2) +
				'%)'}
			value={(totalUHBAvailable - totalHUBWon).toString()}
			animated={true}
		/>
		<!-- TOP Mints-->
		<StatCard
			label={'SharesNFT Prizes Left' +
				' (' +
				((100 * (10 - topMints?.length)) / 10).toFixed(2) +
				'%)'}
			value={10 - topMints?.length + ' $CHOS'}
			animated={true}
		/>
		<StatCard
			label={'Minted Hubbys' + ' (' + ((100 * (allMints?.length || 0)) / 5000).toFixed(2) + '%)'}
			value={`${allMints?.length || 0} / 5000`}
			animated={true}
		/>

		<!-- participants -->
		<StatCard label="Participants" value={stats?.length} animated={true} />
	</div>
		...
		>
			{#if animated}
					...
					{:else}
						<div>
							<div class="flex flex-col gap-2 p-2">
								<div class="rounded-xl bg-base-200 p-0">
									<div
										class="w-full h-48 rounded-md bg-base-200 flex justify-center items-center border border-base-300"
									>
										<div class="flex justify-center flex-col items-center gap-2">
											<p>No Top Mints Yet</p>
											<a
												class="btn btn-primary font-extrabold"
												href="https://opensea.io/collection/crypto-hubby-s/drop"
												target="_blank"
											>
												Mint Now <ExternalLinkIcon class="w-4 h-4 ml-1" />
											</a>
										</div>
									</div>
								</div>
				...
				<div class="w-full hidden md:block">
					...
					>
						<div
							class=" basis-[48%] flex justify-center p-4 gap-1 flex-col items-center bg-base-200 shadow-md rounded-lg border border-base-300"
						>
